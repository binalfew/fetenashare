...
using Fetenashare.Flogger.Formatters;
...
namespace Fetenashare.Flogger
{
    public static class SerilogHelpers
    {
            ...
            IServiceProvider provider, string applicationName, IConfiguration config)
        {
            ...
            loggerConfig
                ...
                .WriteTo.Logger(lc => lc
                    ...
                    .WriteTo.MSSqlServer(
                        connectionString: config["LoggingConnectionString"],
                        tableName: "PerfLog",
                        autoCreateSqlTable: true,
                        columnOptions: GetSqlColumnOptions()))
                .WriteTo.Logger(lc => lc
                    .Filter.ByIncludingOnly(Matching.WithProperty("UsageName"))
                    .WriteTo.Elasticsearch(new ElasticsearchSinkOptions(new Uri(config["ElkUrl"]))
                        {
                            AutoRegisterTemplate = true,
                            AutoRegisterTemplateVersion = AutoRegisterTemplateVersion.ESv6,
                            IndexFormat = "usage-{0:yyyy.MM.dd}",
                            InlineFields = true,
                            CustomFormatter = new CustomElasticsearchJsonFormatter(inlineFields: true, renderMessageTemplate: false)
                    }
                    ))
                .WriteTo.Logger(lc => lc
        ...
        {
            var excluded = new List<string> {"iat", "nbf", "exp", "auth_time", "amr", "sub" };
